...
// gdal_calc.py, the `gdal raster`/`gdal vector` subcommands, ogr2ogr), libvips (blur),
// tippecanoe + tile-join (tile/pack) and versatiles (pack).
...
// 2. Produce the EPSG:3857 world raster for the given zoom level → data/worldcover-3857.tif.
//
// The top level (MAXLEVEL) is reprojected from the full source mirror: the tiles are
// mosaicked with a VRT and warped to sizeForLevel² covering the standard Mercator square
// (±MERC m), resampled "mode" (dominant class — the only correct choice for categorical
// data). Lower levels are produced by halving the next-higher level's raster (also "mode"),
// which is far cheaper than re-warping every source tile — so the build runs the levels
// from MAXLEVEL down to 0. No nodata is set on the raster, so ESA 0 (the no-data/water
// channel) is counted in the mode at every level (mode resampling skips nodata pixels).
export async function reproject(level) {
	const size = sizeForLevel(level);

	// lower levels: downscale the previous (higher) level's raster by 50%
	if (level < MAXLEVEL) {
		if (!existsSync(file.warped)) throw new Error(`missing ${file.warped} — reproject level ${level + 1} first`);
		console.error('Downscaling worldcover to z%d (%d×%d, mode)', level, size, size);
		const tmp = file.warped + '.tmp.tif';
		...
		await fs.rename(tmp, file.warped); // can't read+write the same file in place
		return;
	}
	...
	console.error('Warping z%d to EPSG:3857 %d×%d → %s', level, size, size, file.warped);
	await run('gdalwarp', [
		'-t_srs',
		'EPSG:3857',
		'-te',
...
export async function channels() {
	...
	await pMap(
		...
		CONCURRENCY,
		async ({ c, i }) => {
			// always (re)build: the raster's resolution changes per level, so a mask from a
			// previous level must not be reused
...
// curved class boundaries with shared borders staying exact. vips streams and works in
// 8-bit; `approximate` precision is fastest. The blur is approximate — fine, since the
// result only feeds an argmax. vips strips the GeoTIFF georeferencing; the argmax step
// re-attaches it.
export async function blur() {
	...
	console.error('Blurring %d masks (σ=%s px, %d workers)', channelDefs.length, BLUR_RADIUS, CONCURRENCY);
	const bar = progress(channelDefs.length, 'Blur');
	await pMap(
		channelDefs.map((_, i) => i),
		CONCURRENCY,
		async (i) => {
			// always (re)blur: the mask's resolution changes per level. vips writes a
			// deflate-compressed 8-bit TIFF.
			await runQuiet('vips', [
				'gaussblur',
				maskPath(i),
				`${blurPath(i)}[compression=deflate,predictor=horizontal]`,
				String(BLUR_RADIUS),
				'--precision',
				PRECISION,
			]);
...
// 7. Build the single-zoom tileset for the given level → data/landcover-z{level}.mbtiles.
//
// One zoom per call (the pack step merges them). Oversized tiles stay coverage-complete by
// simplifying harder and merging the smallest polygons (--coalesce-smallest-as-needed)
// rather than dropping features.
export async function tile(level) {
	const zoom = String(level);
	const out = tilesForLevel(level);
	...
	console.error('Tiling zoom %s → %s', zoom, out);
