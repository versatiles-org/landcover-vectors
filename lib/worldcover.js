// Shared helpers for importing ESA WorldCover 2021 (v200) from AWS Open Data.
...
export const srcdir = path.join(tiledir, 'esa-worldcover-src'); // local reduced-resolution mirror (EPSG:4326)
export const workdir = path.join(tiledir, 'esa-worldcover'); // XYZ raster pyramid output
...
export const runQuiet = (cmd, args) =>
	new Promise((resolve, reject) => {
...
export const pMap = async (items, concurrency, worker) => {
	let i = 0;
	const next = async () => {
...
export const listSourceKeys = async () => {
	const keys = [];
	let token;
	do {
